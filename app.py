...
def _resolve_cache(tile_size: int) -> dict:
    """
    Cache-only mode:
    Load an existing cache NPZ by tile size from CACHE_DIR.
    Does NOT rebuild; if missing, raises FileNotFoundError.
    """
    os.makedirs(CACHE_DIR, exist_ok=True)
    pattern = os.path.join(CACHE_DIR, f"tiles_s{int(tile_size):02d}_allrotflips_*.npz")
    matches = sorted(glob.glob(pattern))
    if not matches:
        raise FileNotFoundError(
            f"No cache found for tile size {tile_size}. "
            f"Expected a file like {os.path.basename(pattern)} in {CACHE_DIR}/"
        )
    path = matches[-1]  # pick the newest/last
...
def _list_available_caches() -> str:
    """Human-readable summary of which caches are present."""
    if not os.path.isdir(CACHE_DIR):
        return "Cache-only mode: no cache directory found."
    lines = ["Cache-only mode: scanning cache folder…"]
    found_any = False
    for s in (4, 8, 16, 32):
        patt = os.path.join(CACHE_DIR, f"tiles_s{s:02d}_allrotflips_*.npz")
        m = sorted(glob.glob(patt))
        if m:
            found_any = True
            lines.append(f"• Found cache(s) for {s}×{s}: {len(m)} file(s)")
        else:
            lines.append(f"• Missing cache for {s}×{s}")
    if not found_any:
        lines.append("⚠️ No caches found. Please include prebuilt NPZ files in the 'cache/' folder.")
    return "\n".join(lines)
...
def build_mosaic(
    ...
    """Returns: processed preview, mosaic, metrics md, timing md, download path."""
    try:
        # normalize image
        if isinstance(img, str):
            img = np.asarray(Image.open(img).convert("RGB"), dtype=np.uint8)
...
"""
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                run = gr.Button("Build Mosaic", variant="primary")
                download = gr.DownloadButton(label="Download Mosaic")
                status = gr.Textbox(label="Status", value="", interactive=False, lines=8)
        ...
        # Cache-only status on load (no building)
        demo.load(lambda: _list_available_caches(), outputs=[status])

        run.click(
            fn=build_mosaic,
            inputs=[img, grid, tile, method, quant, alpha_slider],
            outputs=[out_proc, out_mosaic, out_metrics, out_timing, download],
        )
